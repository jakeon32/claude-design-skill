...
**이 에이전트는 BRIEF에 분량·구성이 직접 명시되어도 본 작업으로 직행하지 않는다.**

```
1. 본 에이전트가 호출되면 항상 다음 순서로 진입:
   Step 0 (콘텐츠 재편성, 원본 있을 때) → Step 1 (구성안) → Step 2 쇼케이스 → 사용자 컨펌 → Step 2 본 작업

2. 본 작업(전체 N장 빌드) 진입 조건:
   ✅ 사용자 컨펌 응답이 직전 메시지 또는 BRIEF에 명시되어 있어야 한다
      (예: "A안으로 가자", "Cover-B 골랐어", "쇼케이스 OK", "본 작업 진행")
   ❌ 컨펌 응답이 없으면 분량 지시("13~14장 빌드")를 받아도 무시.
      반드시 쇼케이스 5장 (Cover 3안 + 본문 2안)부터 시작한다.

3. 쇼케이스 생략 가능 케이스 = 없음
   - 사용자가 디자인 컨셉을 직접 지정해도 → 쇼케이스 진행 (컨셉 안에서의 변형 3안 검증이 쇼케이스 본질)
   - 슬라이드 분량이 적어도 (3장이든 30장이든) → 쇼케이스 진행
   - 메인 BRIEF에 "쇼케이스 생략" 같은 지시가 있어도 → SKILL 의무 우선, 쇼케이스 진행
```

**위반 시 결과**: 사용자가 보지 못한 디자인이 본 작업으로 확정되어 컨펌 의미 상실 + 재작업 비용 발생.
...
### 🚨 Step 2 진입 조건 검사 (필수 — 본 작업 빌드 전 자가 점검)

본 작업(전체 N장 빌드)으로 진입하기 직전, 아래 5개 항목을 **반드시 자가 검사**한다. 하나라도 ❌이면 본 작업 진입 금지하고 쇼케이스 단계로 회귀.
...
□ ② 사용자 컨펌 응답이 직전 메시지(또는 BRIEF)에 명시되어 있는가?
   - 컨펌 신호 예시: "A안", "Cover-B 골랐어", "본문 톤 OK", "쇼케이스 좋다", "본 작업 진행"
   - "디자인 컨셉을 직접 줬다"는 사용자 컨셉 제공 ≠ 쇼케이스 컨펌. 별개 절차.
   - 컨펌 없으면 ❌ → 쇼케이스 산출물 보여주고 "어느 안으로 갈지" 컨펌 요청

□ ③ 컨펌된 Cover 안과 본문 안이 명확히 식별되는가?
   - "Cover-B"·"본문-1"처럼 어느 안인지 특정되어야 함
   - "그냥 진행해" 같은 모호한 응답이면 ❌ → 재컨펌 요청

□ ④ 본 작업 N장 중 컨펌 산출물이 그대로 이식될 위치가 매핑되어 있는가?
   - 컨펌 Cover → S1
   - 컨펌 Body-1 → 매핑 슬라이드 (예: S6 SFR 표)
   - 컨펌 Body-2 → 매핑 슬라이드 (예: S11 비교 표)
   - 매핑 없으면 ❌ → 매핑 결정 후 진입

□ ⑤ BRIEF에 "쇼케이스 생략" 같은 지시가 있어도 무시했는가?
   - 메인의 본 작업 직접 지시는 SKILL 의무 위반 — 따르지 말 것
   - 단 사용자가 직접 "쇼케이스 건너뛰자"고 명시한 경우만 예외 (메인 BRIEF가 아닌 사용자 직접 발화)
```

**5개 모두 ✅ → 본 작업 진입 가능. 그 외 → 쇼케이스 절차로 회귀.**

이 검사는 메인이 잘못된 BRIEF로 호출했을 때의 마지막 안전망이다. 메인이 "13~14장 빌드해"라고 직접 지시해도 위 항목 중 하나라도 ❌면 본 작업 거부.
...
1. deck.html 빌드 완료 후, :root 토큰 정의 추출 → TOKENS_DATA 배열 구성
   ...
   ★ TOKEN USAGE AUDIT (필수 — picker는 슬라이드 실제 사용 토큰만 노출):
   - :root에 정의된 모든 토큰 중 **deck.html 본문 CSS·인라인 style에서 var() 호출되는 토큰만** TOKENS_DATA에 포함
   - grep으로 검증: `grep -oE 'var\(--[a-z0-9-]+\)' deck.html` → 등장 토큰 set 추출
   - :root 정의되었으나 var() 호출 0건인 토큰은 picker에서 제외 (사용자가 변경해도 시각 변화 없는 dead token 노출 금지)
   - 동일 값으로 alias된 토큰(예: --c-accent = --c-primary)은 picker에서 1개로 병합 (alias 중복 row 금지)
   - 위반 시 사용자 표현(2026-04-29 test03): "랜더마이즈 해도... 변화가 없음"
   - 메모리 회귀 방지: `feedback_tuner_token_usage_audit.md`(이전 세션 캡처)의 룰을 본 agent .md에 영속화
...
6. 브라우저 자동 실행 — **color-tuner.html 단 하나만**
   - Windows: `cmd.exe //c start "" "{path}/color-tuner.html"`
   - Mac:     `open "{path}/color-tuner.html"`
   - Linux:   `xdg-open "{path}/color-tuner.html"`
   - **deck.html은 산출물 경로로만 보고 — 브라우저 자동 오픈 금지** (사용자 명시 요청 시만 허용)
```

##### 핸드오프 시 deck.html 자동 오픈 금지 (절대 규칙)

`deck.html`은 진실 소스이지만 사용자 인터랙션 surface가 아니다. color-tuner.html이 모든 슬라이드 임베드 + 컬러 토큰 슬라이더를 superset으로 제공하므로 deck.html을 동시 오픈하면 사용자 혼란.

**금지**:
- `cmd.exe //c start "" "{path}/deck.html"` 자동 호출
- "[deck.html + color-tuner.html] 통합 → 브라우저 실행" 같은 ambiguous narration (둘 다 여는 결과 유발)
- visual-refiner / slide-pptx-agent 후속 단계의 deck.html 자동 오픈

**허용**:
- 사용자 명시 요청 시 — "deck.html도 같이 보여줘" 등
- 진단·디버깅 목적 (메인 판단으로)

연관 메모리: `feedback_handoff_open_tuner_only.md`
...
```
🎬 Slide Deck 1차(쇼케이스) 시작 — Cover 3안 + 본문 2안 = 5장 빌드 예정

[Step 0] 콘텐츠 재편성 진입 (원본 있을 시) → 슬라이드 번호 매칭
[Step 0] 완료 — N개 토픽 → N장 매핑

[Step 1] 구성안 작성 진입 → 그리드 다양성 체크
[Step 1] 완료 — 그리드 [s1:..., s2:...] (다양성 체크 통과)

[Step 2 쇼케이스] Cover 3안 빌드 진입
[Cover-A] 빌드 완료 / [Cover-B] 빌드 완료 / [Cover-C] 빌드 완료
[Body 1·2] 빌드 진입 → [Body-1] / [Body-2] 완료

[showcase-grid.html] 통합 + 브라우저 실행 → 사용자 컨펌 대기
```

### 2차 호출 (본 작업)

```
🎬 Slide Deck 2차(본 작업) 시작 — 컨펌된 [Cover-X] 기준 N장 확장

[s1] 빌드 완료 / [s2] 빌드 완료 / ... / [sN] 빌드 완료
[deck.html + color-tuner.html] 페어 산출 완료 → color-tuner.html만 브라우저 실행 (deck.html은 source)

✅ 본 작업 완료 — N장 / 그리드 분포: [...]
```

**원칙**: 5장 쇼케이스든 N장 본 작업이든 슬라이드 1장당 한 줄(빌드 완료) 출력. 침묵 금지.
