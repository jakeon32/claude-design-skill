...
```
STEP A. 스타일 파일 의무 로드
  → 스타일 지정됨: 해당 references/styles/[style].md 실제로 읽기
  → 스타일 미지정: style-recommender로 top 3 추천 → 확인 후 로드
  → base theme (light/dark) 명시적으로 확인

STEP B. 디자인 시스템 선언 (사용자 확인 필수)
  → 색상, 폰트, 간격, 이미지 전략, base theme 구두 선언
  → "이 방향으로 진행할까요?" → 사용자 OK 받기 전까지 생성 금지

STEP C. 단위 설계 선언 — 모드별 에이전트로 이관
  → 확정된 모드에 따라 해당 에이전트의 STEP C 규칙을 따른다:

  ① Prototype  → prototype-agent.md "단위 선언" 섹션
     - 화면/컴포넌트마다 생성 전: 레이아웃, 컴포넌트 목록, 반응형 여부 선언
     - 사용자 확인 → HTML/컴포넌트 작성

  ② Slide Deck → slide-deck-agent.md "슬라이드 단위 선언" 섹션
     - 슬라이드마다 생성 전: 레이아웃 유형, 텍스트 요소 수 확인 (Cover ≤ 2개)
     - base theme 준수, anti-slop 체크 → 사용자 확인 → HTML 작성

  ③ From Template → template-agent.md 참조
     - 소스 파일 분석 → 구조 매핑 선언 → 확인 후 재건

  ④ Other → other-agent.md 참조
     - 출력 형식(이메일/배너/인포그래픽 등) 명시 → 단위별 선언

  ⑤ Document → document-agent.md 참조
     - 페이지마다: 페이지 유형(A–F), 정보 계층(L1–L7) 선언 → 확인 후 생성
```

**왜 이 게이트가 필요한가**: 트리거 이후 패턴 기반 즉흥 생성으로 넘어가면
규칙 위반이 기본값이 된다. 선언 → 확인 → 생성 순서가 유일한 차단 방법.
각 에이전트 파일이 그 모드의 유일한 생성 규칙 소스다 — SKILL.md는 흐름만 정의한다.
