...
[메인 자동 후속 — 2차 종료 직후 즉시 발동]
  ...
  → STEP 5: visual-refiner 위임 (5차원 평가 + 자동 수정)
  → STEP 6: 핸드오프
  메인이 위 3단계를 자체 처리로 대체 금지 — 각 sub-agent를 명시적으로 호출.
```
...
| 5 | 시각 QA + 5차원 평가 + 자동 수정 | visual-refiner | `agents/visual-refiner.md` |
| 6 | 핸드오프 | (메인) | DESIGN_SYSTEM + 산출물 경로 + QA 리포트 |

세부 절차는 owner 에이전트.md / reference 단일 출처. SKILL.md는 흐름만 정의 (MANDATORY GATE 참조).
Step 4·5는 메인의 자동 호출 의무 — 직접 자체 처리 금지.
...
1. **TodoWrite 선행** — 2개 이상 phase가 예상되는 작업은 시작 시 phase 전체를 TodoWrite로 등록.
   Slide Deck 모드 표준 phase: BRIEF / DESIGN_SYSTEM / 쇼케이스 / 본 작업 / **slide-qa(코드)** / **visual-refiner(시각)** / 핸드오프 (★ QA를 단일 phase로 묶지 말 것 — 두 sub-agent 분리)
