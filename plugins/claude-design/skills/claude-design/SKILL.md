...
STEP 3. 모드별 에이전트 위임 (Agent tool 호출 — 4모드 통일)
  ...
  ② Slide Deck    → Agent tool 호출 (subagent_type: "slide-deck-agent") ★ 2회 호출 패턴 — 아래 참조
...
```
[1차 호출] subagent_type: "slide-deck-agent"
...
[2차 호출] subagent_type: "slide-deck-agent" (또는 1차 agentId에 SendMessage)
